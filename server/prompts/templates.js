...
` : ''}
现在立即开始输出，以"===== 简历正文 ====="开头：`;
  return { system: GENERATION_SYSTEM, user: prompt };
}
...
- 去掉LinkedIn链接`;
  ...
  prompt += `\n\n【已预置的CSS样式说明】：
系统已预置专业简洁的CSS：Arial/PingFang SC字体、10.5pt正文、1.35行距、2pt段间距、h1=16pt、h2=11pt带下划线、.contact-info居中。
请直接使用这些样式对应的HTML标签，不要自行添加内联样式（除非用户格式要求中有特殊需求）。`;
