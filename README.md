...
| JD 图片 OCR（支持本地识别及可选 AI 直接识别） | 管理素材库内容本身 |
| 多模型并行评审并打分 | 自动投递简历（V3规划） |
| 跨投递一致性自动检查 | ATS评分预测 |
| 多轮AI辅助编辑（带上下文记忆） | 简历模板市场 |
| 导出可打印的HTML文件 | 长期职业规划 |
| 灵活配置多供应商AI模型 | 内容审查或过滤AI输出 |
| PII自动脱敏保护（姓名/邮箱/电话/社交链接） | |
| 导出预处理文本素材库（供其他AI工具直接使用；默认排除明显 prompt/review/export artifact，并完整纳入Essay/PRD/Spec/项目经历等原始工作素材后再去重） | |
...
项目维护了一份综合测试套件 (`test-e2e.mjs`)，涵盖 E2E 回归测试、mock 单元测试（OpenAI-Compat 缓存行为、State.js 加密、推理强度参数映射等），用于验证核心 API 路由和内部模块的功能。

**运行测试前的准备工作：**

1. **确保 `.env` 文件存在**：项目根目录必须包含 `.env` 文件，其中包含 `GEMINI_KEY`：
   ```
   GEMINI_KEY=your-google-ai-studio-api-key
   ```
   `.env` 文件已在 `.gitignore` 中被忽略，不会提交到仓库（出于安全考虑）。

2. **启动服务器**：测试需要连接到本地运行的服务器：
   ```bash
   npm run dev
   ```
   默认服务器端口为 `3001`，测试会连接到 `http://localhost:3001/api`。
...
**测试运行机制：**
- 测试脚本会自动从项目根目录的 `.env` 文件加载 `GEMINI_KEY`
- 如果 `GEMINI_KEY` 已通过环境变量设置，则跳过 `.env` 加载
- 测试使用绝对路径定位 `.env` 文件，不依赖 `process.cwd()`，确保任何目录下运行都能正确加载

**测试输出示例：**
```
=== Resume Tailor Lean E2E ===

[PASS] base /init ready connections=1
...
**常见问题排查：**
- **错误：请设置环境变量 GEMINI_KEY**
  - 检查 `.env` 文件是否存在且包含 `GEMINI_KEY`
  - 确认 `.env` 文件格式正确（`GEMINI_KEY=xxx`，无多余空格）
- **错误：无法连接 Gemini API**
  - 检查服务器是否正在运行 (`npm run dev`)
  - 确认 VPN 连接（Google AI Studio 在中国大陆需要 VPN）
- **网络问题导致失败**
  - 测试已内置重试机制，等待 15 秒后自动重试
  - 配额错误会等待 5 秒后触发模型自动切换
...
| **Generator** | 根据JD+素材库+指令生成定制简历和求职信；承担 JD 解析 AI 兜底 | 旗舰推理模型 | 需要最高写作质量；可配置推理强度 |
| **Reviewer × N** | 独立评审+打分（支持多个模型并行）；首个 Reviewer 默认承担评审合并/Review 对话 | 多种模型混搭 | 多模型交叉评审降低偏见；可配置推理强度 |
| **Format Converter** | 纯文本→可打印HTML排版；在本地 OCR 质量差时作为 JD 图片 OCR 的 AI 兜底 | 免费/轻量模型 | 简单任务和 OCR 兜底都适合低成本模型 |
| **Preprocessor** | 素材库 AI 预处理（可选） | 免费/轻量模型 | 素材库智能整理优化，适合低成本模型 |
...
- **生成指令**和**评审指令**独立配置——用户可分别为生成和评审环节设定不同的自定义要求
...
用户拿到JD → 粘贴JD文本或上传 1..N 张 JD 图片 → **可选"直接用 AI 识别"**（通过 Format Converter）或执行本地 OCR 提取文本（必要时再用 AI 补救）→ 选择基础简历 → 加载素材库 → **使用指令区的'加载文件'按钮加载之前保存的指令模板**（如专门针对特定岗位类型的生成指令、评审指令、HTML格式指令）→ 点击"生成简历" → AI流式生成定制简历/求职信 → 用户编辑 → 点击"开始Review" → AI多模型评审打分 → **使用'保存文件'按钮保存当前自定义指令供后续复用** → 点击"采纳并更新" → 满意后点击"排版" → 系统隐去代码结构并在幽灵容器内构建闭环 HTML DOM → 自动直接唤起操作系统级别的保存 PDF 对话框 → **点击"打开PDF"按钮在新标签页中查看PDF文件内容** → 一键保存矢量无损简历 → 完成。

### 场景5：导出简历素材文件库

用户加载简历素材库后，点击"导出预处理文本素材库"按钮 → 系统本地估算总输入 Token 数量（基于tiktoken经验：中文≈1 token/汉字，其他≈4字符/1 token）→ 生成包含所有去重后素材的纯文本文件 → **在导出文件和状态栏中显示预估输入 Token 数量** → 用户可将此文件直接用于其他 AI 工具（如本地 RAG 系统）→ 完成。

### 场景6：使用 AI 预处理素材库

用户勾选"使用 AI 预处理" → **可选：在预处理指令区输入自定义预处理要求**（或使用默认 prompt）→ 点击"导出预处理文本素材库" → AI 读取原始素材库内容 → **智能整理、优化素材结构** → 在预处理对话区显示处理过程和 Token 统计（sourceTokens / digestTokens）→ 保存预处理结果到缓存（下次素材未变时复用）→ 导出预处理文本文件 → 完成。
...
**失败回退**：AI 预处理失败时自动回退到本地预处理，并在系统消息中提示。
