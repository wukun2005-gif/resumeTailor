...
用户拿到JD → 粘贴JD文本或上传 1..N 张 JD 图片 → **可选"直接用 AI 识别"**（通过 Format Converter）或执行本地 OCR 提取文本（必要时再用 AI 补救）→ 选择基础简历 → 加载素材库 → 点击"生成简历" → AI流式生成定制简历/求职信 → 用户编辑 → 点击"开始Review" → AI多模型评审打分 → 点击"采纳并更新" → 满意后点击"排版" → 系统隐去代码结构并在幽灵容器内构建闭环 HTML DOM → 自动直接唤起操作系统级别的保存 PDF 对话框 → **点击"打开PDF"按钮在新标签页中查看PDF文件内容** → 一键保存矢量无损简历 → 完成。
