...
### V1（当前版本）：单应用内的多Agent编排 + PII脱敏保护

多Agent协作（Orchestrator/Generator/Reviewer/HTML Converter）在一个本地应用内完成简历定制全流程，并内置PII脱敏保护——详见[第7节：安全、隐私与合规](#7-安全隐私与合规)。**已完成并上线。**
...
```mermaid
flowchart LR
    V1["V1 ✅: 多Agent编排<br/>+ PII脱敏保护"] --> V2["V2: 成本感知Agent<br/>+ 多模态感知Agent"]
