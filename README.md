...
```mermaid
flowchart TD
    LIB[(本地简历素材库)] -->|自动读取所有素材<br/>段落去重+缓存| APP
...
用户拿到JD → 粘贴JD文本或上传 1..N 张 JD 图片 → 本地 OCR 提取文本（必要时由 Format Converter 做一次性 AI OCR 兜底）→ 选择基础简历 → 加载素材库 → 点击"生成简历" → AI流式生成定制简历/求职信 → 用户编辑 → 点击"开始Review" → AI多模型评审打分 → 点击"采纳并更新" → 满意后点击"生成HTML并下载" → 浏览器打印PDF → 完成。
...
### V1（当前版本）：单应用内的多Agent编排 + PII脱敏保护 + Token/费用透明化 + 图片JD输入

三个可配置 Agent（Generator / Reviewer / Format Converter）加上一层内部编排逻辑，在一个本地应用内完成简历定制全流程，并内置 PII 脱敏保护——详见[第8节：安全、隐私与合规](#8-安全隐私与合规)。同时，系统已经把 Token/费用透明化落到了产品层：每次调用后显示 input/output token、按连接折算费用、显示会话累计成本，并在模型发现阶段展示 RPM/RPD/TPM 配额信息。当前版本还新增了**图片型 JD 输入**：支持上传 1..N 张社交媒体平台导出的职位截图或普通 `JPG/PNG/WebP` 图片，默认先做本地 OCR，仅在质量差且用户主动触发时，才由 Format Converter 执行一次性 AI OCR 兜底。**已完成并上线。**
