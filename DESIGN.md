...
|------|------|----------|-------------|
| 2026-04-23 | 修复 sourceTokens 为0的bug：缓存命中时重新计算 sourceTokens，兼容旧缓存格式 | Bug 修复 | - |
| 2026-04-23 | 导出功能显示源文件和预处理后token：导出"素材库预处理文本"文件时显示两个token统计 | UI/功能增强 | - |
| 2026-04-23 | 改进本地token估算：基于tiktoken经验的新算法，中文≈1 token/汉字 | UI/功能增强 | - |
| 2026-04-23 | 指令区增加文件加载功能：生成/评审/HTML指令区支持文件加载/保存（已修复文件读取bug） | UI/功能增强 | - |
...
| 2026-04-22 | apply-review 缺失指令修复：在 diff 模式下添加 instructions 参数 | Bug 修复 | - |
| 2026-04-22 | 状态展示位置移动：从底部移至标题旁 | UI 改进 | - |
...
| 2026-04-23 | AI 预处理素材库功能：新增 Preprocessor Agent、预处理指令区、预处理对话区、缓存隔离 | 功能增强 | - |
 ...
 | 2026-04-24 | E2E 测试框架整改：移除无效测试、精简断言、添加测试分类指南文档 | 测试框架优化 | - |
 | 2026-04-24 | 修复 Reviewer agent 模型配置回归问题：取消选择后重新打开仍被默认选中 | Bug 修复 | - |
| 2026-04-24 | 修复 AI 预处理缓存测试：parseSSEText 添加 fromCache 字段支持，改用真实 API 测试缓存 | Bug 修复 | - |
