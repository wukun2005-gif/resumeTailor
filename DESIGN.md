...
### 设置弹窗
- 宽度 820px (`.modal-wide`)
- **Tab 切换**：4 个 tab（模型连接 / Agent 分配 / GitHub / 隐私），默认显示"模型连接"；tab 选择状态通过 localStorage 持久化
- **关闭按钮固定**：`.modal-header` 使用 `position: sticky` 确保滚动时标题栏和 X 按钮始终可见；点击外部区域不关闭弹窗
- **模型连接配置**（模型连接 tab）：3 个可折叠的供应商区块，每块一个表格（模型类型 / URL / Key / Model ID）
- **Agent 模型分配**（Agent 分配 tab）：5 个选择器（Orchestrator / Generator / Reviewer / Format Converter / Preprocessor），从已配置连接中动态生成选项；推理强度下拉默认隐藏在"高级选项"折叠区内
...
|------|------|----------|-------------|
| 2026-05-10 | U0/U1 设置弹窗交互修复 + tab 切换精简：modal-header sticky 固定关闭按钮；点击外部不关闭弹窗；设置弹窗改为 4 tab（模型连接/Agent 分配/GitHub/隐私）；推理强度下拉移入"高级选项"折叠区；tab 选择状态 localStorage 持久化 | index.html, src/main.js, src/style.css, DESIGN.md | b1f75b9 |
