...
|------|------|----------|-------------|
| 2026-05-10 | U0/U1 设置弹窗交互修复 + tab 切换精简：modal-header sticky 固定关闭按钮；点击外部不关闭弹窗；设置弹窗改为 4 tab（模型连接/Agent 分配/GitHub/隐私）；推理强度下拉移入"高级选项"折叠区；tab 选择状态 localStorage 持久化 | index.html, src/main.js, src/style.css | |
